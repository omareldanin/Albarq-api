    ...
    model: {
      $allModels: {
        async findManyPaginated<Model, Args>(
          this: Model,
          args: Prisma.Exact<Args, Prisma.Args<Model, "findMany">>,
          pagination: {page: number; size: number; withCount?: boolean},
        ): Promise<{
          ...
          const data = await (this as any).findMany(findArgs);
          ...
          const dataCount = await (this as any).count({
          ...
          return {
            data,
            dataCount,
            pagesCount: calculatePagesCount(dataCount, pagination.size),
          };
        },
      },
    },
