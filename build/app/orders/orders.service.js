...
class OrdersService {
    ...
    createPaperOrder = async (data) => {
        ...
        if (checkOrders.length > 0) {
            throw new AppError_1.AppError("تم اضافة الطلب مسبقا", 404);
        }
        if (!client) {
    ...
    forwardOrderToCompany = async (data) => {
        ...
        if (checkOrders.length > 1) {
            ...
            if (oldOrder.forwarded && oldOrder.company.id === data.companyId) {
                throw new AppError_1.AppError("تم احالة الطلب مسبقا", 400);
            }
    ...
    bulkForwardOrdersToCompany = async (data) => {
        ...
        const newCompany = await db_1.prisma.company.findUnique({
            where: {
                id: data.companyId,
            },
        });
        const oldCompany = await db_1.prisma.company.findUnique({
            where: {
