...
class OrdersRepository {
    ...
    async getProfits(order) {
        ...
        if (order?.branch?.id === order?.client.branchId) {
            ...
            const branchsCost = await db_1.prisma.branch.findUnique({
            ...
            const receivingDeliveryCosts = branchsCost?.forwardedDeliveryCosts;
            const forwardedDeliveryCosts = company?.governoratesDeliveryCosts;
            receivingBranchNet =
    ...
    async createOrder(data) {
        ...
        let deliveryCost = await this.getDeliverCost(data.clientID, data.orderData.governorate, branch?.isChild ? branch.parentBranchId : data.orderData.branchID);
        const governoratesDeliveryCosts = client.governoratesDeliveryCosts;
        if (governoratesDeliveryCosts) {
            deliveryCost =
                governoratesDeliveryCosts.find((governorateDeliveryCost) => {
        ...
        const where = data.loggedInUser?.role === "INQUIRY_EMPLOYEE"
            ...
            : {
                AND: [
                    ...
                    {
                        totalCost: data.filters.limit === 0
                            ? 0
    ...
    async getBranchsOrdersCount(data) {
        ...
        const createdAtEndFilter = data.filters.endDate
            ...
            : {};
        // ── Count helpers ───────────────────────────────────────────────
        const countForwarded = (id) => {
            const childBranchs = childMap.get(id) ?? [];
            return db_1.prisma.order.count({
                where: {
                    AND: [
                        { confirmed: true },
        ...
        const [forwaredResults, receivedResults] = await Promise.all([
            Promise.all(branchsIds.map((id) => limit(async () => ({ id, count: await countForwarded(id) })))),
            Promise.all(branchsIds.map((id) => limit(async () => ({ id, count: await countReceived(id) })))),
        ]);
    ...
    async updateOrdersCosts2(data) {
        ...
        if (data.costs.reportType === client_1.ReportType.CLIENT) {
            for (const order of data.orders) {
                ...
                const cost = order?.branch?.isChild
                    ? order?.client.branchCosts.find((c) => c.branchId === order.branch?.parentBranchId)
                    : order?.client.branchCosts.find((c) => c.branchId === order.branch?.id);
                ...
                else if (!data.costs.baghdadDeliveryCost &&
                    !data.costs.governoratesDeliveryCost) {
                    const governoratesDeliveryCosts = order?.client
                        .governoratesDeliveryCosts;
                    if (governoratesDeliveryCosts) {
        ...
        =============================== */
        if (data.costs.reportType === client_1.ReportType.DELIVERY_AGENT) {
            // build response
            for (const order of data.orders) {
                ...
                if (order?.forwardedFrom.activeProfit) {
                    const cost = order.forwardedFrom.branchCosts?.find((c) => c.branchId === order.branch?.id);
                    if (cost) {
                        deliveryAgentNet =
  ...
  `;
    ...
    async getOrdersStatistics(data) {
        ...
        const isManagerOrMainRepo = loggedInUser?.role === "COMPANY_MANAGER" || loggedInUser?.mainRepository;
        let branchConstraintsA = [];
        let branchConstraintsB = [];
        if (filters.orderType === "forwardedAll" &&
            isManagerOrMainRepo &&
            filters.branchID) {
            branchConstraintsA = [
        ...
        if (filters.orderType === "receivedAll" &&
            isManagerOrMainRepo &&
            filters.branchID) {
            branchConstraintsB = [
        ...
        else if (loggedInUser?.role !== "COMPANY_MANAGER" &&
        ...
        const isClientAssistant = loggedInUser.role === "CLIENT_ASSISTANT" ||
            loggedInUser.role === "EMPLOYEE_CLIENT_ASSISTANT";
        ...
        const filtersReformed = data.loggedInUser.role === "INQUIRY_EMPLOYEE"
            ...
            : {
                AND: [
                    ...
                    {
                        clientId: loggedInUser.role === "RECEIVING_AGENT"
                            ? { in: filters.inquiryClientsIDs || [] }
