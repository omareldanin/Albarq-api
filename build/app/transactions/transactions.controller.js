...
class TransactionsController {
    createTransaction = (0, catchAsync_1.catchAsync)(async (req, res) => {
        const transactionData = transactions_dto_1.TransactionCreateSchema.parse(req.body);
        const loggedInUser = res.locals.user;
        const companyId = loggedInUser.companyID;
        transactionData.createdById = loggedInUser.id;
        const createdTransaction = await transactionsRepository.createTransaction(companyId, loggedInUser.branchId, transactionData);
        res.status(200).json({
            status: "success",
            data: createdTransaction,
        });
    });
    getEmployeesWallet = (0, catchAsync_1.catchAsync)(async (req, res) => {
        const loggedInUser = res.locals.user;
        const companyId = loggedInUser.companyID;
        let page = req.query.page ? +req.query.page : 1;
        let size = req.query.size ? +req.query.size : 10;
        const result = await transactionsRepository.getCompanyNetGroupedByCreatedBy(companyId, page, size);
        res.status(200).json({
            status: "success",
            page: result.page,
    ...
    getReceivingAgent = (0, catchAsync_1.catchAsync)(async (req, res) => {
        const { receivingAgentId, start_date, end_date } = req.query;
        const loggedInUser = res.locals.user;
        let startDate = new Date();
        let endDate = new Date();
        const isMainRepository = loggedInUser?.mainRepository || loggedInUser?.role === "COMPANY_MANAGER";
        ...
        const ordersCount = await db_1.prisma.order.groupBy({
            by: ["clientId"],
            _count: {
                id: true,
            },
        ...
        const deliveredOrdersCount = await db_1.prisma.order.groupBy({
            by: ["clientId"],
            _count: {
                id: true,
            },
        ...
        const insideOrders = await db_1.prisma.order.groupBy({
            by: ["clientId"],
            _sum: {
                companyNet: true,
                clientNet: true,
        ...
        res.status(200).json({
            ...
            data: clients.map((client) => {
                const count = ordersCount.find((c) => c.clientId === client.id)?._count
                    .id;
                const count2 = deliveredOrdersCount.find((c) => c.clientId === client.id)?._count.id;
                ...
                forwardedReports.forEach((report) => {
                    const clientOrders = report.orders.filter((o) => o.clientId === client.id);
    ...
    updateTransaction = (0, catchAsync_1.catchAsync)(async (req, res) => {
        const transactionId = +req.params.transactionId;
        const transactionData = transactions_dto_1.TransactionUpdateSchema.parse(req.body);
