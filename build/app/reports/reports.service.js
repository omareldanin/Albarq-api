...
class ReportsService {
    ...
    async getAllReports(data) {
        ...
        if (data.loggedInUser.role !== client_1.EmployeeRole.COMPANY_MANAGER &&
            ...
            else if (data.loggedInUser.role === "REPOSITORIY_EMPLOYEE" &&
    ...
    async deactivateReport(data) {
        ...
        if (!report)
            return;
        if (!data.loggedInUser.showTreatury &&
        ...
        if (data.loggedInUser.role !== "COMPANY_MANAGER") {
            if ((report?.type === "CLIENT" &&
                ...
                (report?.type === "DELIVERY_AGENT" &&
                    !data.loggedInUser.permissions.includes("DELETE_DELIVERY_AGENT_REPORT")) ||
